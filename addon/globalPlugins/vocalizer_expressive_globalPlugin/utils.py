...
class VocalizerOpened(object):
	def __enter__(self):
		...
		if speech.getSynth().name != vocalizer_expressive.SynthDriver.name and _reentrancy == 0:
			try:
	...
	def __exit__(self, exc_type, exc_val, exc_tb):
		...
		from synthDrivers.vocalizer_expressive import _vocalizer
		_reentrancy -= 1
		if speech.getSynth().name != vocalizer_expressive.SynthDriver.name and _opened and _reentrancy == 0:
			try:
