...
def callback(instance, outDev, message, userData):
	...
	try:
		...
		if speakingInstance is None and messageType != VE_MSG_ENDPROCESS:
			...
			outData.contents.ulMrkListLen = c_uint(markBufSize * sizeof(VE_MARKINFO))
			return NUAN_OK
...
def open(voice=None):
	...
	# Open tts instance
	instance = VE_HSAFE()
	veDll.ve_ttsOpen(hSpeechClass, installResources.hHeap, installResources.hLog, byref(instance), None)
...
def getVoiceList(languageName):
	...
	veDll.ve_ttsGetVoiceList(hSpeechClass, c_char_p(languageName), 0, byref(voiceInfos), byref(nItems))
	l = []
	for i in range(nItems.value):
